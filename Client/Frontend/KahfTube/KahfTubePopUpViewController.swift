...
class KahfTubePopUpViewController: UIViewController, PopoverContentComponent {
  ...
  var safegazeSettingsChanged: ((KahfTubePopUpViewController, BraveShield) -> Void)?
  ...
  override func loadView() {
      ...
      newView.reloadWebView = {
          self.tab.webView?.reload()
      }
      view = newView
  }
...
extension KahfTubePopUpViewController {
  class View: UIView {
    ...
    init(frame: CGRect, url: URL?, tab: Tab) {
      ...
      stackView.addArrangedSubview(KahfTubeView.redirect(url: url, updateView: { [self] in
