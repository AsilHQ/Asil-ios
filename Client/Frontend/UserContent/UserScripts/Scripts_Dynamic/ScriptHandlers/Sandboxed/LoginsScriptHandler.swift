...
class LoginsScriptHandler: TabContentScript {
  ...
  fileprivate func promptSave(_ login: LoginData) {
    ...
    snackBar = TimerSnackBar(text: promptMessage, img: UIImage(sharedNamed: "brave.logo")!)
  ...
  private func addSnackBarForPrompt(for login: PasswordForm, isUpdating: Bool, _ completion: @escaping () -> Void) {
    ...
    snackBar = TimerSnackBar(
      text: promptMessage,
      img: isUpdating ? UIImage(named: "key", in: .module, compatibleWith: nil)! : UIImage(sharedNamed: "brave.logo")!)
